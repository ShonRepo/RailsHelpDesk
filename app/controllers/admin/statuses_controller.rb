class Admin::StatusesController < Admin::BaseController
  ...
  def create
    ...
    if @Status.save
      redirect_to admin_statuses_path, notice: 'Статус успешно создан'
    else
      add_breadcrumb "новый статус", new_admin_status_path, title: 'Статусы'
  ...
  def update
    if@Status.update(status_params)
      redirect_to admin_statuses_path, notice: 'статус успешно изменен'
    else
      breadcrumb_update
  ...
  def destroy
    if @Status.destroy
      redirect_to admin_statuses_path, notice: 'статус удален'
    else
      redirect_to admin_statuses_path, alert: 'не удалось'
